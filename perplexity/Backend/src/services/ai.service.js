  ...
  try {
    const response = await mistralModel.invoke([
      new SystemMessage(
        ...
        `Output ONLY 2-5 short bullet points with the key facts relevant to the query. ` +
        `No headings, no source names, no raw tables, no URLs, no markdown formatting, no extra commentary. ` +
        `Ignore junk fragments like "Temporarily Unavailable" or broken symbols. Plain text bullets only, each starting with "- ".`
      ),
...
export async function* generateResponseStream(messages) {
  ...
  for await (const [chunk, metadata] of stream) {
    // Debug — pehli baar chalate waqt ye uncomment karo taaki asli node names dikhein
    // console.log("node:", metadata?.langgraph_node, "| content:", chunk?.content, "| tool_calls:", chunk?.tool_call_chunks);

    const isToolCallChunk = chunk?.tool_call_chunks?.length > 0;

    // Sirf actual text content chahiye, tool-call ke internal chunks nahi — node-name pe depend nahi karna
